...
### 位置接尾辞処理の非対称性（`_found_in_scope_ex` L679）

`_LOC_SUFFIXES = {'内', '外', '上', '下', '中', '間', '側', '前', '後', '先'}`

**照応詞側のみ** 末尾剥ぎを行う。先行詞側には行わない。

| 方向 | 例 | 動作 | 意図 |
|------|-----|------|------|
| 照応詞に「側」あり | 先行詞「表面」← 照応「前記表面側」 | 末尾「側」剥ぎ → マッチ ✅ | 「表面」は導入済み。「側」は限定修飾 |
| 先行詞に「側」あり | 先行詞「一表面側」← 照応「前記一表面」 | マッチしない → error ✅ | 「一表面」は未導入。error が正しい |

逆方向（先行詞「一表面側」→「前記一表面」）を許容すると、「入力側」を導入しただけで
「前記入力」が通ってしまうため、意図的に片方向設計にしている。
特許5421742（パナソニック トランス、2013）の「前記一表面」errorはこの仕様による正検出。
