...
SECTION_PATTERNS = {
    ...
    "claims":      r"【特許請求の範囲】",
    "drawings":    r"【図面の簡単な説明】",
    # 背景技術・先行技術文献は本願自身の図面符号の説明対象ではないため
    # 独立キーに分離（未分離のままだと直前セクションに紛れ込み、
    # 背景技術中の他社文献の符号言及が本願の符号として誤って扱われる）
    "background":  r"【背景技術】|【先行技術文献】",
    # 技術分野・課題・解決手段・効果は独立キーには分離するが、いずれの
    # チェックからも参照しない（"捨てバケツ"）。これらの見出しを認識せず
    # 放置すると、未認識の見出しが直前セクション（background 等）に
    # 際限なく雪崩れ込んでしまう。特に「課題を解決するための手段」は
    # 出願人自身の解決手段の記載（請求項に近い内容）であり、これが
    # background に混入すると本願自身の記載を他社文献の符号言及として
    # 誤って警告してしまう
    "misc_front":  (r"【技術分野】|【発明が解決しようとする課題】"
                    r"|【課題を解決するための手段】|【発明の効果】"),
    # 「発明の詳細な説明」「発明を実施するための形態」「実施例Ｎ」両方を description に
    "description": (r"【発明の詳細な説明】|【発明を実施するための形態】"
                     r"|【発明を実施するための最良の形態】|【発明の実施の形態】"
                     r"|【実施例[０-９0-9]*】"),
